 ...
 * @unrestricted
 */
Flamechart.TimelineFlameChartView = class extends UI.VBox {
  /**
  ...
  constructor(delegate) {
    ...
    this._mainDataProvider = new Flamechart.TimelineFlameChartDataProvider();
    this._mainDataProvider.addEventListener(
        Flamechart.TimelineFlameChartDataProvider.Events.DataChanged, () => this._mainFlameChart.scheduleUpdate());
    ...
    // this._networkDataProvider = new Flamechart.TimelineFlameChartNetworkDataProvider();
    ...
    // this._networkSplitWidget.setSidebarSize(0);
    ...
    // this._countersView = new Flamechart.CountersGraph(this._delegate);
    this._chartSplitWidget.setMainWidget(this._mainFlameChart);
    ...
    this._mainDataProvider.setEventColorMapping(Flamechart.TimelineUIUtils.eventColor);
  ...
  _updateColorMapper() {
    ...
    if (!this._model)
      return;
    const colorByProduct = this._groupBySetting.get() === Flamechart.AggregatedTimelineTreeView.GroupBy.Product;
    this._mainDataProvider.setEventColorMapping(
        colorByProduct ? this._colorByProductForEvent.bind(this) : Flamechart.TimelineUIUtils.eventColor);
  ...
  _colorByProductForEvent(event) {
    return Flamechart.TimelineUIUtils.eventColorByProduct(
  ...
  updateRangeSelection(startTime, endTime) {
    this._delegate.select(Flamechart.TimelineSelection.fromRange(startTime, endTime));
  }
    ...
    if (this._model) {
      this._eventListeners = [
        ...
        this._model.addEventListener(
            Flamechart.PerformanceModel.Events.ExtensionDataAdded, this._appendExtensionData, this)
      ];
  ...
  highlightEvent(event) {
    const entryIndex =
        event ? this._mainDataProvider.entryIndexForSelection(Flamechart.TimelineSelection.fromTraceEvent(event)) : -1;
    if (entryIndex >= 0)
  ...
  _updateSearchResults(shouldJump, jumpBackwards) {
    ...
    if (!this._searchRegex || !this._model)
      return;
    const regExpFilter = new Flamechart.TimelineFilters.RegExp(this._searchRegex);
 ...
 * @unrestricted
 */
Flamechart.TimelineFlameChartView.Selection = class {
  /**
   * @param {!Flamechart.TimelineSelection} selection
 ...
 * @unrestricted
 */
Flamechart.TimelineFlameChartMarker = class {
  /**
   * @param {number} startTime
   * @param {number} startOffset
   * @param {!Flamechart.TimelineMarkerStyle} style
   */
...
/** @enum {string} */
Flamechart.TimelineFlameChartView._ColorBy = {
  URL: 'URL',
  Product: 'Product'
};
