 ...
 */
Flamechart.TimelineTreeView = class extends UI.VBox {
  constructor() {
    super();
  ...
  /**
   * @protected
   * @return {?Flamechart.PerformanceModel} model
   */
  model() {
    return this._model;
  }
  ...
  init() {
    ...
    this._textFilter = new Flamechart.TimelineFilters.RegExp();
  ...
  /**
   * @protected
   * @return {!Flamechart.TimelineFilters.RegExp}
   */
  textFilter() {
  ...
  populateToolbar(toolbar) {
    ...
    /**
     * @this {Flamechart.TimelineTreeView}
     */
  ...
  refreshTree() {
    ...
    for (const child of children.values()) {
      ...
      const gridNode =
          new Flamechart.TimelineTreeView.TreeGridNode(child, totalUsedTime, maxSelfTime, maxTotalTime, this);
  ...
  _sortingChanged() {
    ...
    function compareNumericField(field, a, b) {
      const nodeA = /** @type {!Flamechart.TimelineTreeView.TreeGridNode} */ (a);
      const nodeB = /** @type {!Flamechart.TimelineTreeView.TreeGridNode} */ (b);
    ...
    function compareName(a, b) {
      const nodeA = /** @type {!Flamechart.TimelineTreeView.TreeGridNode} */ (a);
      const nodeB = /** @type {!Flamechart.TimelineTreeView.TreeGridNode} */ (b);
      const nameA = Flamechart.TimelineTreeView.eventNameForSorting(nodeA._profileNode.event);
      const nameB = Flamechart.TimelineTreeView.eventNameForSorting(nodeB._profileNode.event);
  ...
  _updateDetailsForSelection() {
    const selectedNode = this._dataGrid.selectedNode ?
        /** @type {!Flamechart.TimelineTreeView.TreeGridNode} */ (this._dataGrid.selectedNode)._profileNode :
        null;
  ...
  _onMouseMove(event) {
    const gridNode = event.target && (event.target instanceof Node) ?
        /** @type {?Flamechart.TimelineTreeView.TreeGridNode} */ (
   ...
   * @param {!TimelineModel.TimelineProfileTree.Node} treeNode
   * @protected
   * @return {?Flamechart.TimelineTreeView.GridNode}
   */
  ...
  performSearch(searchConfig, shouldJump, jumpBackwards) {
    ...
    this._searchResults =
        this._root.searchTree(event => Flamechart.TimelineUIUtils.testContentMatching(event, searchRegex));
 ...
 * @unrestricted
 */
Flamechart.TimelineTreeView.GridNode = class extends DataGrid.SortableDataGridNode {
  /**
   ...
   * @param {!Flamechart.TimelineTreeView} treeView
   */
  ...
  _createNameCell(columnId) {
    ...
    if (this._profileNode.isGroupNode()) {
      const treeView = /** @type {!Flamechart.AggregatedTimelineTreeView} */ (this._treeView);
      ...
      name.textContent = Flamechart.TimelineUIUtils.eventTitle(event);
      ...
      const eventStyle = Flamechart.TimelineUIUtils.eventStyle(event);
 ...
 * @unrestricted
 */
Flamechart.TimelineTreeView.TreeGridNode = class extends Flamechart.TimelineTreeView.GridNode {
  /**
   * @param {!TimelineModel.TimelineProfileTree.Node} profileNode
   * @param {number} grandTotalTime
   * @param {number} maxSelfTime
  ...
  constructor(profileNode, grandTotalTime, maxSelfTime, maxTotalTime, treeView) {
    ...
    profileNode[Flamechart.TimelineTreeView.TreeGridNode._gridNodeSymbol] = this;
  }
  ...
  populate() {
    ...
    for (const node of this._profileNode.children().values()) {
      const gridNode = new Flamechart.TimelineTreeView.TreeGridNode(
...
Flamechart.TimelineTreeView.TreeGridNode._gridNodeSymbol = Symbol('treeGridNode');

/**
 * @unrestricted
 */
Flamechart.AggregatedTimelineTreeView = class extends Flamechart.TimelineTreeView {
  constructor() {
    super();
    this._groupBySetting =
        Common.settings.createSetting('timelineTreeGroupBy', Flamechart.AggregatedTimelineTreeView.GroupBy.None);
    this._groupBySetting.addChangeListener(this.refreshTree.bind(this));
    this.init();
    this._stackView = new Flamechart.TimelineStackView(this);
    this._stackView.addEventListener(
        Flamechart.TimelineStackView.Events.SelectionChanged, this._onStackViewSelectionChanged, this);
   ...
   * @param {string} name
   * @return {string}
   * @this {Flamechart.AggregatedTimelineTreeView}
   */
  _beautifyDomainName(name) {
    if (Flamechart.AggregatedTimelineTreeView._isExtensionInternalURL(name))
      name = Common.UIString('[Chrome extensions overhead]');
    else if (Flamechart.AggregatedTimelineTreeView._isV8NativeURL(name))
  ...
  populateToolbar(toolbar) {
    ...
    const groupBy = Flamechart.AggregatedTimelineTreeView.GroupBy;
    const options = [
  ...
  /**
   * @param {!Flamechart.AggregatedTimelineTreeView.GroupBy} groupBy
  ...
  _groupingFunction(groupBy) {
    ...
    switch (groupBy) {
      ...
      case GroupBy.EventName:
        return event => Flamechart.TimelineUIUtils.eventStyle(event).title;
      case GroupBy.Category:
        return event => Flamechart.TimelineUIUtils.eventStyle(event).category.name;
  ...
  _domainByEvent(groupSubdomains, event) {
    ...
    if (!url)
      return '';
    if (Flamechart.AggregatedTimelineTreeView._isExtensionInternalURL(url))
      return Flamechart.AggregatedTimelineTreeView._extensionInternalPrefix;
    if (Flamechart.AggregatedTimelineTreeView._isV8NativeURL(url))
      return Flamechart.AggregatedTimelineTreeView._v8NativePrefix;
   ...
   */
  _appendContextMenuItems(contextMenu, node) {
    if (this._groupBySetting.get() !== Flamechart.AggregatedTimelineTreeView.GroupBy.Frame)
      return;
    if (!node.isGroupNode())
      return;
  ...
  static _isExtensionInternalURL(url) {
    return url.startsWith(Flamechart.AggregatedTimelineTreeView._extensionInternalPrefix);
  }
  ...
  static _isV8NativeURL(url) {
    return url.startsWith(Flamechart.AggregatedTimelineTreeView._v8NativePrefix);
  }
};

Flamechart.AggregatedTimelineTreeView._extensionInternalPrefix = 'extensions::';
Flamechart.AggregatedTimelineTreeView._v8NativePrefix = 'native ';

/**
 * @enum {string}
 */
Flamechart.AggregatedTimelineTreeView.GroupBy = {
  None: 'None',
  EventName: 'EventName',
  Category: 'Category',
  Domain: 'Domain',
 ...
 * @unrestricted
 */
Flamechart.CallTreeTimelineTreeView = class extends Flamechart.AggregatedTimelineTreeView {
  constructor() {
    super();
 ...
 * @unrestricted
 */
Flamechart.BottomUpTimelineTreeView = class extends Flamechart.AggregatedTimelineTreeView {
  constructor() {
    super();
 ...
 * @unrestricted
 */
Flamechart.TimelineStackView = class extends UI.VBox {
  ...
  setStack(stack, selectedNode) {
    ...
    for (const node of stack) {
      const gridNode = new Flamechart.TimelineTreeView.GridNode(node, totalTime, totalTime, totalTime, this._treeView);
  ...
  selectedTreeNode() {
    const selectedNode = this._dataGrid.selectedNode;
    return selectedNode && /** @type {!Flamechart.TimelineTreeView.GridNode} */ (selectedNode)._profileNode;
  }

  _onSelectionChanged() {
    this.dispatchEventToListeners(Flamechart.TimelineStackView.Events.SelectionChanged);
  }
};

/** @enum {symbol} */
Flamechart.TimelineStackView.Events = {
