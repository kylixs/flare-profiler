 ...
 * @unrestricted
 */
Flamechart.TimelineDetailsView = class extends UI.VBox {
  /**
   * @param {!Flamechart.TimelineModeViewDelegate} delegate
   */
  constructor(delegate) {
    ...
    const tabIds = Flamechart.TimelineDetailsView.Tab;
    ...
    /** @type Map<string, Flamechart.TimelineTreeView> */
    this._rangeDetailViews = new Map();

    const bottomUpView = new Flamechart.BottomUpTimelineTreeView();
    ...
    const callTreeView = new Flamechart.CallTreeTimelineTreeView();
    ...
    const eventsView = new Flamechart.EventsTimelineTreeView(delegate);
  ...
  setModel(model, track) {
    if (this._model !== model) {
      if (this._model)
        this._model.removeEventListener(Flamechart.PerformanceModel.Events.WindowChanged, this._onWindowChanged, this);
      this._model = model;
      if (this._model)
        this._model.addEventListener(Flamechart.PerformanceModel.Events.WindowChanged, this._onWindowChanged, this);
    }
    this._track = track;
    this._tabbedPane.closeTabs(
        [Flamechart.TimelineDetailsView.Tab.PaintProfiler, Flamechart.TimelineDetailsView.Tab.LayerViewer], false);
   ...
   */
  _setContent(node) {
    const allTabs = this._tabbedPane.otherTabs(Flamechart.TimelineDetailsView.Tab.Details);
  ...
  _updateContents() {
    ...
    if (view) {
      ...
      view.updateContents(this._selection || Flamechart.TimelineSelection.fromRange(window.left, window.right));
    }
  }
  ...
  setSelection(selection) {
    ...
    switch (this._selection.type()) {
      ...
      case Flamechart.TimelineSelection.Type.Frame:
        ...
        this._setContent(Flamechart.TimelineUIUtils.generateDetailsContentForFrame(frame, filmStripFrame));
        if (frame.layerTree) {
          const layersView = this._layersView();
          layersView.showLayerTree(frame.layerTree);
          if (!this._tabbedPane.hasTab(Flamechart.TimelineDetailsView.Tab.LayerViewer))
            this._appendTab(Flamechart.TimelineDetailsView.Tab.LayerViewer, Common.UIString('Layers'), layersView);
        }
        break;
      case Flamechart.TimelineSelection.Type.NetworkRequest:
  ...
  _layersView() {
    ...
    this._lazyLayersView =
        new Flamechart.TimelineLayersView(this._model.timelineModel(), this._showSnapshotInPaintProfiler.bind(this));
    return this._lazyLayersView;
  }

  /**
   * @return {!Flamechart.TimelinePaintProfilerView}
   */
  _paintProfilerView() {
    if (this._lazyPaintProfilerView)
      return this._lazyPaintProfilerView;
    this._lazyPaintProfilerView = new Flamechart.TimelinePaintProfilerView(this._model.frameModel());
    return this._lazyPaintProfilerView;
  }
  ...
  _showSnapshotInPaintProfiler(snapshot) {
    ...
    if (!this._tabbedPane.hasTab(Flamechart.TimelineDetailsView.Tab.PaintProfiler)) {
      this._appendTab(
          Flamechart.TimelineDetailsView.Tab.PaintProfiler, Common.UIString('Paint Profiler'), paintProfilerView, true);
    }
    this._tabbedPane.selectTab(Flamechart.TimelineDetailsView.Tab.PaintProfiler, true);
  }
  ...
  _showEventInPaintProfiler(event) {
    ...
    if (this._tabbedPane.hasTab(Flamechart.TimelineDetailsView.Tab.PaintProfiler))
      return;
    this._appendTab(
        Flamechart.TimelineDetailsView.Tab.PaintProfiler, Common.UIString('Paint Profiler'), paintProfilerView);
  }
  ...
  _updateSelectedRangeStats(startTime, endTime) {
    if (!this._model || !this._track)
      return;
    const aggregatedStats = Flamechart.TimelineUIUtils.statsForTimeRange(this._track.syncEvents(), startTime, endTime);
    ...
    const pieChart = Flamechart.TimelineUIUtils.generatePieChart(aggregatedStats);
 ...
 * @enum {string}
 */
Flamechart.TimelineDetailsView.Tab = {
  Details: 'Details',
  EventLog: 'EventLog',
  CallTree: 'CallTree',
  BottomUp: 'BottomUp',
