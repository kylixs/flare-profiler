 ...
 * @unrestricted
 */
Flamechart.TimelineLoader = class {
  /**
   * @param {!Flamechart.TimelineLoader.Client} client
   */
  constructor(client) {
    this._client = client;
    ...
    this._state = Flamechart.TimelineLoader.State.Initial;
    this._buffer = '';
   ...
   * @param {!File} file
   * @param {!Flamechart.TimelineLoader.Client} client
   * @return {!Flamechart.TimelineLoader}
   */
  static loadFromFile(file, client) {
    const loader = new Flamechart.TimelineLoader(client);
    const fileReader = new Bindings.ChunkedFileReader(file, Flamechart.TimelineLoader.TransferChunkLengthBytes);
  ...
  write(chunk) {
    ...
    if (this._state === Flamechart.TimelineLoader.State.Initial) {
      if (chunk.startsWith('{"nodes":[')) {
        this._state = Flamechart.TimelineLoader.State.LoadingCPUProfileFormat;
      } else if (chunk[0] === '{') {
        this._state = Flamechart.TimelineLoader.State.LookingForEvents;
      } else if (chunk[0] === '[') {
        this._state = Flamechart.TimelineLoader.State.ReadingEvents;
      } else {
    ...
    if (this._state === Flamechart.TimelineLoader.State.LoadingCPUProfileFormat) {
      this._buffer += chunk;
      return Promise.resolve();
    }

    if (this._state === Flamechart.TimelineLoader.State.LookingForEvents) {
    ...
    if (this._state !== Flamechart.TimelineLoader.State.ReadingEvents)
      return Promise.resolve();
    if (this._jsonTokenizer.write(chunk))
      return Promise.resolve();
    this._state = Flamechart.TimelineLoader.State.SkippingTail;
    if (this._firstChunk)
...
Flamechart.TimelineLoader.TransferChunkLengthBytes = 5000000;

/**
 * @interface
 */
Flamechart.TimelineLoader.Client = function() {};

Flamechart.TimelineLoader.Client.prototype = {
  loadingStarted() {},
 ...
 * @enum {symbol}
 */
Flamechart.TimelineLoader.State = {
